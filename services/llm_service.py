...
def get_llm_response(text: str) -> str:
    ...
    try:
        res = requests.post(GEMINI_URL, headers=HEADERS, params=params, json=payload, timeout=TIMEOUT)
        res.raise_for_status()
        result = _parse_gemini_response(res.json())
        logger.info("✅ LLM text response received")
        return result
    except requests.RequestException as e:
        logger.error(f"LLM text request failed: {e}")
...
def get_llm_response_with_history(history: list) -> str:
    ...
    params = {"key": GEMINI_API_KEY}
    try:
        res = requests.post(GEMINI_URL, headers=HEADERS, params=params, json={"contents": history}, timeout=TIMEOUT)
        res.raise_for_status()
        result = _parse_gemini_response(res.json())

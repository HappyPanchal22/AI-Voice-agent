...
from schemas.responses import ChatResponse, TTSResponse
...
# ----------------- HELPERS -----------------
def fallback_response(transcription: str = "", error_detail: str = None) -> dict:
    msg = FALLBACK_TEXT
    return {
...
async def chat_with_agent(session_id: str, file: UploadFile):
    ...
    try:
        bot_response = get_llm_response_with_history(
            [{"role": "user" if m["role"] == "user" else "model", "parts": [{"text": m["content"]}]} for m in conversation]
        )
    except Exception as e:
        logger.error(f"LLM error: {e}")
        return fallback_response(user_message, error_detail="Gemini API error")
    ...
    try:
        ...
        return fallback_response(user_message, error_detail="Murf API error")
